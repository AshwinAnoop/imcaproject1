...
<body>
	...
	$idmin = 0;
	$result = mysqli_query( $conn, "SELECT MAX(medbill_id) FROM medbill WHERE medbill_date='$bookdate'" );
	
	$idmax = $result->fetch_array()[0] ;
	$result = mysqli_query( $conn, "SELECT MIN(medbill_id) FROM medbill WHERE medbill_date='$bookdate'" );
